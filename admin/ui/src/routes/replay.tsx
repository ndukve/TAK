...
function ReplayPage() {
  ...
  return (
    <Layout>
      <div className="p-6">
        <PageHeader eyebrow="OPERATIONS / REPLAY" title="Replay Mode" />

        {!status.service_cert_ready ? (
          <div className="rounded-lg border border-zinc-200 dark:border-white/10 hud-glass p-6">
            ...
            <button onClick={handleSetup} disabled={busy}
              className="flex items-center gap-2 px-4 py-2 bg-accent-fill hover:bg-accent-fill-hover text-accent-text text-sm rounded-lg disabled:opacity-50 transition-colors">
              <Icon name="settings-3-line" size={14} /> Set Up Replay
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="rounded-lg border border-zinc-200 dark:border-white/10 hud-glass p-4">
            ...
            <div className="rounded-lg border border-zinc-200 dark:border-white/10 hud-glass p-4">
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                {[1, 2, 5, 10].map(s => (
                  <button key={s} onClick={() => setSpeed(s)} disabled={status.playback}
                    className={`px-2 py-1 rounded-lg text-xs disabled:opacity-50 ${speed === s ? 'bg-accent-fill text-accent-text' : 'bg-zinc-200 dark:bg-zinc-950 text-zinc-700 dark:text-zinc-300'}`}>
                    {s}x
                  </button>
                ))}
              </div>
              {status.playback && (
                <button onClick={handleStopPlayback} disabled={busy}
                  className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm rounded-lg disabled:opacity-50 transition-colors mb-3">
                  ...
                  chunks.map(c => (
                    <div key={c.id} className="flex items-center justify-between py-2">
                      ...
                      <button onClick={() => handlePlay(c.id)} disabled={status.playback || busy}
                        className="p-1.5 rounded-lg hover:bg-zinc-200 dark:hover:bg-white/[0.05] text-accent-ring disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-accent-ring"
