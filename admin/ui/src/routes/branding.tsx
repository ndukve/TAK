      ...
      <div className="p-6 max-w-lg">
        ...
        <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                <input type="color" value={accentFill} onChange={e => setAccentFill(e.target.value)}
                  className="w-10 h-9 rounded-lg border border-zinc-300 dark:border-white/10 bg-zinc-200 dark:bg-zinc-950" />
          ...
          <button type="submit" disabled={saving}
            className="px-4 py-2 rounded-lg bg-accent-fill hover:bg-accent-fill-hover text-accent-text text-sm disabled:opacity-50">
            {saving ? 'Saving…' : 'Save'}
          </button>
        </form>
        ...
        <div className="mt-8 pt-6 border-t border-zinc-200 dark:border-white/10 space-y-2">
          ...
          {logoUrl ? (
            <div className="flex items-center gap-3">
              <img src={logoUrl} alt="Current logo" className="w-16 h-16 rounded-lg border border-zinc-300 dark:border-white/10 object-contain bg-zinc-200 dark:bg-zinc-950" />
              <button onClick={handleLogoRemove} className="px-3 py-1.5 rounded-lg bg-zinc-300 dark:bg-zinc-800 hover:bg-zinc-400 dark:hover:bg-zinc-700 text-sm">Remove</button>
            </div>
          ) : (
          ...
          <button type="button" onClick={() => fileRef.current?.click()}
            className="px-3 py-1.5 rounded-lg bg-zinc-300 dark:bg-zinc-800 hover:bg-zinc-400 dark:hover:bg-zinc-700 text-zinc-900 dark:text-white text-sm">
            Upload logo
          </button>
        </div>
      </div>
