...
export function ShellSession() {
  ...
  return (
    <div
      className={cn(
        'fixed inset-y-0 right-0 z-10 top-14 md:top-0 md:left-56 left-0 bg-zinc-50 dark:bg-zinc-950 p-6 flex flex-col',
    ...
    >
      ...
      {!ticket && (
        <div className="max-w-sm space-y-4">
          <div className="flex items-start gap-3 p-4 rounded-lg border border-yellow-300 dark:border-yellow-700/50 bg-yellow-50 dark:bg-yellow-900/20">
          ...
          <form onSubmit={elevate} className="space-y-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2 rounded-lg bg-accent-fill hover:bg-accent-fill-hover text-accent-text text-sm font-medium disabled:opacity-50 transition-colors"
            >
      ...
      {ticket && (
        <div
          ref={termRef}
          className="flex-1 rounded-lg overflow-hidden border border-zinc-200 dark:border-white/10"
