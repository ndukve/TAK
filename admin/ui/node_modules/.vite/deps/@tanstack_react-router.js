...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/utils.js
...
function getNodeMatch(path, parts, segmentTree, fuzzy) {
  ...
  while (stack.length) {
    ...
    if (node.kind === 2 && node.route && !isFrameMoreSpecific(bestMatch, frame))
      continue;
    if (node.parse) {
      if (!validateParseParams(path, parts, frame))
        continue;
    ...
    if (isBeyondPath && node.index) {
      ...
      let indexValid = true;
      if (node.index.parse) {
        if (!validateParseParams(path, parts, indexFrame))
          indexValid = false;
      }
      if (indexValid) {
    ...
    if (node.wildcard)
      for (let i = node.wildcard.length - 1; i >= 0; i--) {
        ...
        if (prefix) {
          if (isBeyondPath)
            continue;
          if (!(segment.caseSensitive ? part : lowerPart ?? (lowerPart = part.toLowerCase())).startsWith(prefix))
            continue;
        }
        if (suffix) {
          if (isBeyondPath)
  ...
  if (isServer ?? rest.server) {
    if (path.indexOf("{") === -1) {
      ...
      while (cursor2 < length2) {
        while (cursor2 < length2 && path.charCodeAt(cursor2) === 47)
          cursor2++;
        if (cursor2 >= length2)
          break;
        const start = cursor2;
...
var setupPendingTimeout = (inner, matchId, route, match) => {
  var _a;
  if (match._nonReactive.pendingTimeout !== void 0)
    return;
  ...
  const serverSsr = () => {
    if (isServer ?? inner.router.isServer) {
      ...
      if (isPromise(maybePromise))
        return maybePromise.then(queueExecution);
    }
...
var runLoader = async (inner, matchPromises, matchId, index, route) => {
  ...
  try {
    ...
    try {
      ...
      if (!!(loaderResultIsPromise || route._lazyPromise || route._componentsPromise || route.options.head || route.options.scripts || route.options.headers || match._nonReactive.minPendingPromise))
        inner.updateMatch(matchId, (prev) => ({
          ...prev,
          isFetching: "loader"
        }));
      ...
      if (pendingPromise)
        await pendingPromise;
      if (isNotFound(e))
        await ((_c = (_b = route.options.notFoundComponent) == null ? void 0 : _b.preload) == null ? void 0 : _c.call(_b));
...
var loadRouteMatch = async (inner, matchPromises, index) => {
  ...
  if (shouldSkipLoader(inner, matchId)) {
    if (!inner.router.getMatch(matchId))
...
async function loadMatches(arg) {
  ...
  if (notFoundToThrow) {
    ...
    if (renderedBoundaryIndex === void 0) {
      if (true)
        throw new Error("Invariant failed: Could not find match for notFound boundary");
      invariant();
    }
    ...
    if (!rootMatch.globalNotFound) {
      if ((_b = inner.router.getMatch(rootMatch.id)) == null ? void 0 : _b.globalNotFound)
...
function createBrowserHistory(opts) {
  ...
  win.history.pushState = function(...args) {
    ...
    if (!history2._ignoreSubscribers)
      onPushPop("PUSH");
    return res;
  };
  ...
  */
  constructor(options, getStoreConfig) {
    ...
    this.buildLocation = (opts) => {
      const build = (dest = {}) => {
        ...
        if (destRoute)
          ...
          if (this.options.notFoundRoute && (!destMatchResult.foundRoute || destMatchResult.foundRoute.path !== "/" && destMatchResult.routeParams["**"]))
                  ...
                  this.startViewTransition(async () => {
                    ...
                    this.batch(() => {
                      ...
                      for (const s of this.stores.pendingMatchStores.values())
                        if (s.routeId)
                          pendingRouteIds.add(s.routeId);
                      const activeRouteIds = /* @__PURE__ */ new Set();
                      for (const s of this.stores.matchStores.values())
                        if (s.routeId)
                          activeRouteIds.add(s.routeId);
    ...
    this.resolveRedirect = (redirect2) => {
      ...
      if (redirect2.options.href && !redirect2.options._builtLocation && isDangerousProtocol(redirect2.options.href, this.protocolAllowlist))
        throw new Error(true ? `Redirect blocked: unsafe protocol in href "${redirect2.options.href}". Allowed protocols: ${Array.from(this.protocolAllowlist).join(", ")}.` : "Redirect blocked: unsafe protocol");
    ...
    this.matchRoute = (location, opts) => {
      ...
      if ((opts == null ? void 0 : opts.pending) && this.stores.status.get() !== "pending")
        return false;
...
function buildMiddlewareChain(destRoutes) {
  ...
  for (const route of destRoutes) {
    ...
    if ("search" in routeOptions) {
      if ((_a = routeOptions.search) == null ? void 0 : _a.middlewares)
          ...
          try {
            ...
            if (meta && validated) {
              for (const key in validated)
                if (!(key in result))
                  (meta.defaulted || (meta.defaulted = /* @__PURE__ */ new Map())).set(key, validated[key]);
            }
            return {
              ...result,
              ...validated
...
function setupScrollRestoration(router, force) {
  ...
  router.subscribe("onRendered", (event) => {
    ...
    if (!shouldResetScroll)
      scroll.next = true;
    if (typeof router.options.scrollRestoration === "function" && !router.options.scrollRestoration({ location: router.latestLocation }))
      return;
    ...
    try {
      ...
      if (!windowRestored && hash && hashScrollIntoViewOptions)
        (_a = document.getElementById(hash)) == null ? void 0 : _a.scrollIntoView(hashScrollIntoViewOptions);
    } finally {
      ignoreScroll = false;
    }
  });
  ...
  constructor(options) {
    this.init = (opts) => {
      ...
      if (isRoot)
        ...
        if (true)
          throw new Error(`Invariant failed: Child Route instances must pass a 'getParentRoute: () => ParentRoute' option that returns a Route instance.`);
        invariant();
      }
...
function retainSearchParams(keys) {
  return ({ search, next }) => {
    ...
    if (keys === true) {
      ...
      for (const key of (removed == null ? void 0 : removed.keys()) || [])
        if (explicit2 && hasOwn.call(explicit2, key) || deepEqual(search[key], removed.get(key)))
          delete copy2[key];
      for (const key of meta.removedAny || [])
        delete copy2[key];
      for (const key of ((_a = meta.defaulted) == null ? void 0 : _a.keys()) || [])
        if (key in search && !((_b = meta.removedAny) == null ? void 0 : _b.has(key)) && !(((_c = meta.removed) == null ? void 0 : _c.has(key)) && (explicit2 && hasOwn.call(explicit2, key) || deepEqual(search[key], meta.removed.get(key)))))
          copy2[key] = search[key];
      return copy2;
    }
  ...
  return ({ search, next, meta }) => {
    if (input === true) {
      Object.keys(search).forEach((key) => {
        if (meta)
          (meta.removedAny || (meta.removedAny = /* @__PURE__ */ new Set())).add(key);
      });
      return {};
    }
    const result = { ...next(search) };
    if (Array.isArray(input))
      ...
      Object.entries(input).forEach(([key, value]) => {
        if (deepEqual(result[key], value)) {
          delete result[key];
          if (meta)
            (meta.removed || (meta.removed = /* @__PURE__ */ new Map())).set(key, value);
        }
      });
    return result;
  };
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/awaited.js
var React$12 = __toESM(require_react(), 1);
var import_jsx_runtime = __toESM(require_jsx_runtime(), 1);
function useAwaited({ promise: _promise }) {
  if (reactUse)
    return reactUse(_promise);
...
function Await(props) {
  ...
  if (props.fallback)
    return (0, import_jsx_runtime.jsx)(React$12.Suspense, {
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/CatchBoundary.js
...
function CatchBoundary(props) {
  ...
  return (0, import_jsx_runtime2.jsx)(CatchBoundaryImpl, {
    ...
    children: ({ error, reset }) => {
      if (error)
        return React$13.createElement(errorComponent, {
          error,
          reset
        });
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/ClientOnly.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/routerContext.js
var React$14 = __toESM(require_react(), 1);
var routerContext = React$14.createContext(null);

// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useRouter.js
var React$15 = __toESM(require_react(), 1);
function useRouter(opts) {
  const value = React$15.useContext(routerContext);
  if (true) {
    if (((opts == null ? void 0 : opts.warn) ?? true) && !value)
      console.warn("Warning: useRouter must be used inside a <RouterProvider> component!");
  }
  return value;
}

// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/matchContext.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useMatch.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useLoaderData.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useLoaderDeps.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useParams.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useSearch.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useNavigate.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useRouteContext.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/link.js
  ...
  if (_isServer) {
    ...
    const isActive2 = (() => {
      ...
      if (exact) {
        ...
        if (!(currentPathSplit.startsWith(nextPathSplit) && (currentPathSplit.length === nextPathSplit.length || currentPathSplit[nextPathSplit.length] === "/")))
          return false;
      }
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/route.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/fileRoute.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/lazyRouteComponent.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/not-found.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/ScriptOnce.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/SafeFragment.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/renderRouteNotFound.js
var import_react6 = __toESM(require_react(), 1);
var import_jsx_runtime9 = __toESM(require_jsx_runtime(), 1);
function renderRouteNotFound(router, route, data) {
  if (!route.options.notFoundComponent) {
    if (router.options.defaultNotFoundComponent)
      return (0, import_jsx_runtime9.jsx)(router.options.defaultNotFoundComponent, { ...data });
    if (true) {
      if (!route.options.notFoundComponent)
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/scroll-restoration.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/Match.js
...
var Match = React$111.memo(function MatchImpl({ matchId }) {
  ...
  if (isServer ?? router.isServer) {
    ...
    if (!match2) {
      if (true)
        throw new Error(`Invariant failed: Could not find match for matchId "${matchId}". Please file an issue!`);
      invariant();
    }
...
function OnRendered() {
  ...
  useLayoutEffect2(() => {
    ...
    if (currentResolvedLocation && (!previousResolvedLocation || previousResolvedLocation.href !== currentResolvedLocation.href))
      router.emit({
        type: "onRendered",
...
var MatchInner = React$111.memo(function MatchInnerImpl({ matchId }) {
  ...
  if (isServer ?? router.isServer) {
    ...
    if (match2.status === "error")
      return (0, import_jsx_runtime11.jsx)((route2.options.errorComponent ?? router.options.defaultErrorComponent) || ErrorComponent, {
        error: match2.error,
        reset: void 0,
        info: { componentStack: "" }
      });
  ...
  if (match.status === "error") {
    if (isServer ?? router.isServer)
      return (0, import_jsx_runtime11.jsx)((route.options.errorComponent ?? router.options.defaultErrorComponent) || ErrorComponent, {
        error: match.error,
        reset: void 0,
        info: { componentStack: "" }
      });
...
var Outlet = React$111.memo(function OutletImpl() {
  ...
  if (parentGlobalNotFound) {
    if (!route) {
      if (true)
        throw new Error("Invariant failed: Could not resolve route for Outlet render");
      invariant();
    }
  ...
  if (routeId === rootRouteId)
    return (0, import_jsx_runtime11.jsx)(React$111.Suspense, {
      fallback: pendingElement,
      children: nextMatch
    });
  return nextMatch;
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/Transitioner.js
...
function Transitioner() {
  ...
  useLayoutEffect2(() => {
    if (typeof window !== "undefined" && router.ssr || mountLoadForRouter.current.router === router && mountLoadForRouter.current.mounted)
      return;
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/Matches.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/routerStores.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/router.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/RouterProvider.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/ScrollRestoration.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useBlocker.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useRouterState.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useLocation.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/useCanGoBack.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/Asset.js
...
function Asset(asset) {
  ...
  switch (asset.tag) {
    ...
    case "style":
      if (asset.inlineCss && (process.env.TSS_INLINE_CSS_ENABLED === "true" || process.env.TSS_INLINE_CSS_ENABLED === void 0 && isServer))
        return (0, import_jsx_runtime14.jsx)(InlineCssStyle, {
          attrs,
          nonce,
          children
        });
...
function Script({ attrs, children, preventScriptHoist }) {
  ...
  if ((attrs == null ? void 0 : attrs.src) && typeof children === "string" && children.trim().length)
    console.warn("[TanStack Router] <Script> received both `src` and `children`. The `children` content will be ignored. Remove `children` or remove `src`.");
  React$115.useEffect(() => {
    if (dataScript)
      return;
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/headContentUtils.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/Scripts.js
...
// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/HeadContent.js
var import_react9 = __toESM(require_react(), 1);
var import_jsx_runtime16 = __toESM(require_jsx_runtime(), 1);

// node_modules/.pnpm/@tanstack+react-router@1.170.16_react-dom@18.3.1_react@18.3.1__react@18.3.1/node_modules/@tanstack/react-router/dist/esm/HeadContent.dev.js
